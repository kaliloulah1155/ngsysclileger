<?php
/**
 * Affichage des bo�tes aux lettres d'une autre application de la m�me base
 * 
 * Pas facilement int�grable en standard pour les raisons suivantes
 ...
 * - L'url � pr�ciser pour l'ouverture de la question d�pend du param�trage de la page d'index de l'application externe
 * - Le chargement peut �tre long
 * - Int�ressant quand on est en mode SSO, moins si l'utilisateur doit saisir son mdp sur chaque nouvelle application
 ...
 * On pourrait aussi sp�cifier le profil dans les parametres de la portlet mais cela rendrait difficile un parametrage generique par
 * l'administrateur.
 */
...
if (!defined('APPLI_EXTERNE_BAL') || !isset(APPLI_EXTERNE_BAL[$sAppl])) {
    ...
    $objRetour["msg_erreur"] = "L'application externe n'est pas param�tr�e sur cette application.";
}
else {
    ...
    if ($retour)
    {
    	if(defined('CST_CHARGEMENT_BAL_XML') && strcmp(CST_CHARGEMENT_BAL_XML, "1") == 0)
    	{
            ...
            // --------------------------------
            // Utilisateur et profil par d�faut
...
function afficheBalPortletTpl($jeton, $objBal, $iCptGroup, $blocAParserUpper, &$iCptBal, &$tpl)
{
    ...
    // NB : depend du param�trage de la page d'index
