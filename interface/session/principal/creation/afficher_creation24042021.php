...
if ($retour)
{
    ...
    if ($retour)
    {
		...
		if($_REQUEST['POS_TYPEDOC']=='ABS' OR $_REQUEST['POS_TYPEDOC']=='PRT' OR $_REQUEST['POS_TYPEDOC']=='DEM' OR $_REQUEST['POS_TYPEDOC']=='TMS'  OR $_REQUEST['POS_TYPEDOC']=='JUS' ){
			...
			//Recup�ration des donn�es de l'utlisateurs connect�
				$result = pg_query($conn, "SELECT \"NOM\" , \"PRE\" ,\"FON\", \"DPT\" , \"SLT\" from pos_tab_index_prs WHERE \"LOG\"='".$_SESSION["sess_user_name"]."'");
		...
		if($_REQUEST['POS_TYPEDOC']=='PRT' ){
			...
			$result1=pg_query($conn, "SELECT  COUNT(*) AS nbreemprunt  FROM public.pos_tab_index_prt WHERE \"CRE\"='".$_SESSION["sess_user_name"]."' AND \"ETF\"!='CLOS'");
			while($row1 = pg_fetch_array($result1)){
				$nombreEmp = $row1["nbreemprunt"];
			}

			//montant d'emprunt en cours 
			$result2=pg_query($conn, "SELECT  SUM(\"VEM\") AS mtnemprunt FROM public.pos_tab_index_prt WHERE \"CRE\"='".$_SESSION["sess_user_name"]."'  AND \"ETF\"!='CLOS'");
			while($row2 = pg_fetch_array($result2)){
				$montantEmp = $row2["mtnemprunt"];
			}


			$result3=pg_query($conn, "SELECT \"DFI\" AS datefin FROM public.pos_tab_index_prt WHERE \"DFI\"=(SELECT max(\"DFI\") FROM public.pos_tab_index_prt) AND \"CRE\" ='".$_SESSION["sess_user_name"]."'");
