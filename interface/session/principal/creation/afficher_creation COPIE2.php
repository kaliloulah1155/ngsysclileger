...
if ($retour)
{
    ...
    if ($retour)
    {
		...
		if ($_REQUEST['POS_TYPEDOC']=='COT'){
				$result = pg_query($conn, "SELECT \"MAT\"  from pos_tab_index_prs WHERE \"LOG\"='".$_SESSION["sess_user_name"]."'");
				...
				if (!$result){
					...
					while ($row = pg_fetch_row($result)) {
						// as a string 
						 ...
						 if(isset($matricule) ){
							echo "<input  type='hidden' class='matricule'  value='{$matricule}' />"; 
						 }
					}
				}
		...
		if($_REQUEST['POS_TYPEDOC']=='ABS' OR $_REQUEST['POS_TYPEDOC']=='PRT' ){
			...
			//Recup�ration des donn�es de l'utlisateurs connect�
				$result = pg_query($conn, "SELECT \"NOM\" , \"PRE\" ,\"FON\", \"DPT\"  from pos_tab_index_prs WHERE \"LOG\"='".$_SESSION["sess_user_name"]."'");
