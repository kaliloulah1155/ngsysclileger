...
*
*	Attention :
*	- Aucun support d'assuré (install, utilisation), script proposé 'tel que'
*	- Ce script est livré sans garanties, vous l'utilisez sous votre propre 
*	et entière responsabilité
*	- Ce script est de plus en version alpha, donc non stable...
*/
...
function rtf_get_cell_tab(){
	...
	$this->cell_temp .= $this->cell_deff.$this->para.$this->para_format."\intbl".$this->para_texte."\cell";
...
function rtf_set_ligne_tab($align="l", $border=0, $marges=0, $hauteur_ligne=0, $code=0){
	...
	if(!ereg("^(l|r|c)$", $align))
		return(0);
	if(!ereg("^[0-9]{1,4}([.][0-9]{1,2})?$", $border))
		return(0);
	if(!ereg("^[-]?[0-9]{1,8}([.][0-9]{1,2})?$", $hauteur_ligne))
		return(0);
	if(!ereg("^[0-2]$", $code))
		return(0);
	if(!ereg("^[0-9]{1,8}([.][0-9]{1,2})?$", $marges))
		return(0);
...
function rtf_set_cell_tab($largeur, $valign="t", $border=0, $ombrage=0, $color=-1){
	...
	if(!ereg("^[0-9]{1,8}([.][0-9]{1,2})?$", $largeur))
		return(0);
	if(!ereg("^(t|b|c)$", $valign))
		return(0);
	if(!ereg("^[0-9]{1,5}([.][0-9]{1,2})?$", $border))
		return(0);
	if(!ereg("^[0-9]{1,3}$", $ombrage))
		return(0);
	if(!ereg("^(-1|[0-9]{1,4})$", $color))
		return(0);
...
function rtf_set_para_texte($texte, $font_style=-1, $font_size=0, $font_number=-1, $format=0, $f_color=-1){
	...
	if(!ereg("^([0-9]{1,3}|-1)$", $font_style))
		return(0);
	if(!ereg("^[0-9]{1,4}$", $font_size))
		return(0);
	if(!ereg("^([0-9]{1,3}|-1)$", $font_number))
		return(0);
	if(!ereg("^([biu]+|0)$", $format))
		return(0);
	if(!ereg("^([0-9]{1,3}|-1)$", $f_color))
		return(0);
...
function rtf_set_para_format($code=0, $style=0, $align="l", $l_indent=0, $r_indent=0, $fl_indent=0, $space_b=0, $space_a=0, $space_l=0){
	...
	if(!ereg("^(l|c|j|r|d)$", $align))
		return(0);
	if(!ereg("^[0-9]+([.][0-9]{1,2})?$", $fl_indent))
		return(0);
	if(!ereg("^[0-9]+([.][0-9]{1,2})?$", $l_indent))
		return(0);
	if(!ereg("^[0-9]+([.][0-9]{1,2})?$", $r_indent))
		return(0);
	if(!ereg("^[0-9]+([.][0-9]{1,2})?$", $space_b)) 
		return(0);
	if(!ereg("^[0-9]+([.][0-9]{1,2})?$", $space_a)) 
		return(0);
	if(!ereg("^[-]?[0-9]+([.][0-9]{1,2})?$", $space_l)) 
		return(0);     
	...
	*/
	switch($code){
		case 0: $this->para = "\plain";
			break;

		case 1: $this->para = "\plain";
			break;

		case 2: $this->para = "\pagebb\plain";
			break;
			
		case 3: $this->para = "\pagebb\plain";
			break;
		
		default: $this->para = "\plain"; 
	}
...
function rtf_pictures($path, $pictx=0, $picty=0){
	...
	if(!ereg("^[0-9]{1,10}([.][0-9]{1,2})?$", $pictx))
		return(0);
	if(!ereg("^[0-9]{1,10}([.][0-9]{1,2})?$", $picty))
		return(0);
