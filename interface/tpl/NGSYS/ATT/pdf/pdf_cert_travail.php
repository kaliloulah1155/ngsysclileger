...
class PDF extends FPDF
{
 ...
 $stage='';
  ...
  //fiche personnel
  $query1 ="SELECT \"CIV\", \"NOM\",\"PRE\",\"DNS\",\"FON\",\"LNS\" FROM public.pos_tab_index_prs WHERE  \"MAT\" ='".$matricule."'";
 ...
 $civilite=='M'? $civ='M.':$civ=$civilite;
  ...
  if(!empty($date_entree) AND !empty($date_fin)){
    //echo "Word Found!";
    //PDF STAGE
     ...
     // $pdf->Rect(25 , 25, 160,10,'DF');
      $pdf->Cell(40,10,strtoupper( utf8_decode("CERTIFICAT DE TRAVAIL")),0,1,'C',0 );
  ...
  $pdf->MultiCell(160,10, utf8_decode('    Je soussigné, Monsieur Sébastien ELLEPO , Gérant de la société NGSER, dont le siège social est situé à Abidjan-Plateau , Avenue Lamblin Immeuble MATCA  4ème  étage certifie que  :'));

  $pdf->Ln(5); // Line gap
  $pdf->SetX(20); // abscissa of Horizontal position 
  $pdf->MultiCell(160,10,utf8_decode($civ.' '.$nom.' '.$prenom.' Né le '.$datenaiss.' a été employé de la société NGSER, en qualité de '.$fonction.' du '.$date_entree.' au '.$date_fin.'.'));

  $pdf->Ln(5); // Line gap
  $pdf->SetX(20); // abscissa of Horizontal position 
  $pdf->MultiCell(160,10,utf8_decode($civ.' '.$nom.' '.$prenom.' quitte ce jour la société NGSER, libre de tout engagement.'));

  $pdf->Ln(5); // Line gap
  $pdf->SetX(20); // abscissa of Horizontal position 
  $pdf->MultiCell(160,10,utf8_decode("En foi de quoi, la présente certificat  lui est délivrée pour servir et valoir ce que de droit ."));
  ...
  $pdf->Ln(20);
    ...
    echo "CERTIFICAT DE TRAVAIL INEXISTANT VEUILLEZ REDIGER LE CONTRAT ASSOCIE";
  }
