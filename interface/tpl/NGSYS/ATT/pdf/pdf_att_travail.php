...
class PDF extends FPDF
{
 ...
 $civilite=='M'? $civ='M.':$civ=$civilite;
  ...
  if(!empty($date_entree)){
    ...
    //PDF STAGE
     ...
     // $pdf->Rect(25 , 25, 160,10,'DF');
      $pdf->Cell(40,10,strtoupper( utf8_decode("ATTESTATION DE TRAVAIL")),0,1,'C',0 );
  ...
  $pdf->Ln(10); // Line gap
  $pdf->SetX(20); // abscissa of Horizontal position 
  $pdf->MultiCell(160,10,utf8_decode($civ.' '.$nom.' '.$prenom.' Né le '.$datenaiss.'est embauché depuis le '.$date_entree.'. Il occupe au sein de notre établissement le poste de '.$fonction.'.'));

  $pdf->Ln(10); // Line gap
  $pdf->SetX(20); // abscissa of Horizontal position 
  $pdf->MultiCell(160,10,utf8_decode("En foi de quoi, la présente attestation lui est délivrée pour servir et valoir ce que de droit ."));
  ...
  $pdf->Ln(30);
    ...
    echo "ATTESTATION DE TRAVAIL INEXISTANT VEUILLEZ REDIGER LE CONTRAT ASSOCIE";
  }
