 ...
 $email=$_GET['email'];
	   ...
	   if(!empty($email)){
	    ...
	    $query_san_datemail="
                ...
                \"NOM\",\"PRE\",\"FON\",\"NAT\",\"DAE\",\"NUD\"
				FROM pos_tab_index_san 
				WHERE  \"DAE\" BETWEEN '".$date_deb."' AND '".$date_fin."' AND  \"INT\"='".strtolower($email)."'
	           ";
...
if(isset($export)){
    ...
    $active_sheet->setCellValue('C2',"LISTE DES SANCTIONS");
   ...
   $active_sheet->setCellValue('D4','NATURE DE LA SANCTION');
   $active_sheet->setCellValue('E4','DATE DE LA FAUTE');
   ...
   $active_sheet->setTitle("SANCTION"); //Renomme le worksheet(onglet)
  ...
  $file_name='sanction.'.strtolower($choice);
