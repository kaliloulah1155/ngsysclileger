...
// Send Headers   
   ...
   $query_prs = "SELECT  \"NOM\", \"PRE\",\"MAT\",\"SIF\",\"NAT\",\"CIV\"  FROM public.pos_tab_index_prs WHERE \"MEL\"='".strtolower($email)."'
    ...
    $civilite=='M'? $civ='M.':$civ=$civilite;
      ...
      $query_pay =pg_query("SELECT  \"DCR\",\"PRT\",\"SAB\",\"SRA\",\"ATR\",\"DUR\",\"ACI\",\"RDT\",\"RET\",\"AT2\",\"FAM\",\"TFP\",\"TA\",\"IS\",\"CNV\",\"IGR\",\"ASN\",\"PTE\",\"TRS\",\"SAL\",\"SLB\",\"SLN\"
   FROM public.pos_tab_index_pay WHERE  \"NUD\" ='".$numposeidon."'");
       ...
       $total_brute=0;
        while ($row_pay = pg_fetch_row($query_pay)) {
             ...
             $tot_cot+=((int)$row_pay[8]+(int)$row_pay[9]+(int)$row_pay[10]+(int)$row_pay[11]+(int)$row_pay[12]+(int)$row_pay[13]   );
...
<span>1,20</span>
    ...
    <tr width= "100%"   >
        ...
        <td width= "25%" >
            <span> Retenue de Carburant   </span>
        </td> 
        <td width= "10%">
            <span> ... </span>
        </td>
