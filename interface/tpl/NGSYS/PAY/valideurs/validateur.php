    ...
    //Recuperation des donnés du manager 
     $contests_mng =pg_query("
     	SELECT fpos.\"MEL\",fpos.\"IPO\",prs.\"NOM\",prs.\"PRE\"
			 ...
			 ON fpos.\"MEL\"=prs.\"MEL\"
			 WHERE fpos.\"IPO\" = 'MANAGER ADMINISTRATIF ET JURIDIQUE'  --POSTE MANAGER DTI
     ...
     $contests_mng =pg_query("
       ...
       ") or die('Query failed: ' . pg_last_error());  
     	...
     	while ($row_fnc = pg_fetch_row($contests_mng)) {
    		 $finance=$row_fnc[0];
    		 $fullname_fnc=$row_fnc[2].' '.$row_fnc[3];
		 }
