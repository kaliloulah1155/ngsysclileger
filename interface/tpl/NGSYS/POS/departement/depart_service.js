...
let link_url2 =
    ...
    "/interface/tpl/" +
    appName +
    "/POS/departement/service_actions.php";

//Debut phase 1-Traitement des departements
  ajaxDepartement().then((results) => {

         //departement
        var initdepartement =$('.initdepartement').val();
         for (let i = 0; i < results.length; i++) {
            $("#depart").append(
                "<option value='" + results[i][0] + "' "+(results[i][0]==initdepartement  ? 'selected="selected"' : '')+"    >" + results[i][0] + "</option>"
            );
        }

        //hierarchie operationnelle
         var inithiera_opera =$('.inithiera_opera').val();
         for (let i = 0; i < results.length; i++) {
            $("#hiera_opera").append(
                "<option value='" + results[i][0] + "' "+(results[i][0]==inithiera_opera  ? 'selected="selected"' : '')+"    >" + results[i][0] + "</option>"
            );
        }

        //hierarchie operationnelle
         var inithiera_fonct =$('.inithiera_fonct').val();
         for (let i = 0; i < results.length; i++) {
            $("#hiera_fonct").append(
                "<option value='" + results[i][0] + "' "+(results[i][0]==inithiera_fonct  ? 'selected="selected"' : '')+"    >" + results[i][0] + "</option>"
            );
        }
    ...
    if (depart != "") {
        ...
        ajaxService(depart)
            .then((results) => {
                ...
                var initservice =$('.initservice').val();
...
let selectDepartement = $(".initdepartement").val();
...
if (selectDepartement != "") {
    ...
    ajaxService(selectDepartement)
        .then((results) => {
            for (let i = 0; i < results.length; i++) {
                $("#service").append(
                    "<option value='" + results[i][0] + "' "+(results[i][0]==initservice  ? 'selected="selected"' : '')+"      >" + results[i][0] + "</option>"
                );
            }
        })
        .catch((err) => console.log(err));
