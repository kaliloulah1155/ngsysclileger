...
$(document).on('change', '.personnel', function() {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            $('.get_fonction').val(responseNew[1]);
...
function load_diplome() {
    ...
    var init_diplome = $('.interselectdiplome').val();
...
function load_filiere() {
    var init_filiere = $('.interselectfiliere').val();

    $.ajax({
        url: linkfiliere,
        type: "GET",
        data: {
            init_filiere: init_filiere
        },
        success: function(data) {
...
function load_level() {
    var init_level = $('.interselectlevel').val();
    $.ajax({
        url: linklevel,
        type: "GET",
        data: {
...
function load_domaine() {
    ...
    var init_domaine = $('.interselectdomaine').val();
    $.ajax({
        url: linkdomaine,
        type: "GET",
        data: {
            init_domaine: init_domaine
        },
        success: function(data) {
            $(".domaine").append(
                "<option value='0'>Veuillez s&#233;lectionner</option>"
...
function load_categorie() {
    var init_categorie = $('.interselectcategorie').val();
    $.ajax({
        url: linkcategorie,
        type: "GET",
        data: {
            init_categorie: init_categorie
        },
        success: function(data) {
            $(".categorie").append(
                "<option value='0'>Veuillez s&#233;lectionner</option>"
...
function addRow(data) {
    ...
    html += '</td>' +
        ...
        '<input type="text" onkeypress="return testNum(event, this, 2);" name="montant_prime[]" min="0"  placeholder="Montant de la prime" style="width: 100%;border: none;margin: 2px auto" class="form-control montant_prime">' +
        '</td>' +
        '</tr>';
...
let link_url_rows =
    ...
    "/interface/tpl/" +
    appN +
    "/COT/specifique/prime/rows_contratprime.php";
var dateEnreg1 = $('#date_enreg').val();
//init_email
//alert(dateEnreg1);

function load_table_prime(dateEnreg, user_email) {
    ...
    $.ajax({
        ...
        success: function(resps) {
            ...
            $('.t_total').text('TOTAL PRIME = ' + $('.total').text() + ' FCFA');
...
$(".lib_prime").empty();
load_table_prime(dateEnreg1, init_email);
...
$(document).on('click', '.bouton_sub', function(e) {
    ...
    if (action_modif == 'AA_MOD_COT') {
        ...
        updatePrimeCot(lib_prime, montant_prime, type_contrat, status_activ_cot, personnel, exist_contrat, dateEnreg1);
...
function updatePrimeCot(lib_prime, montant_prime, type_contrat, status_activ_cot, personnel, exist_contrat, dateEnreg1) {

    var link_url_cot_prime_up = "/" +
        appN +
        "/interface/tpl/" +
        appN +
        "/COT/specifique/prime/updated_prime.php";

    $.ajax({
        url: link_url_cot_prime_up,
        type: "GET",
        data: {
            lib_prime: lib_prime,
            montant_prime: montant_prime,
