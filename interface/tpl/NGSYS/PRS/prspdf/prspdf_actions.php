...
// Send Headers
    ...
    require('../../../../../configuration_w/PDF/fpdf.php');
     ...
     while($rowfpos=pg_fetch_row($contests_fposte)){
        ...
        $fpo_fonction=$rowfpos[0];
     }

      $query_contrat="
                SELECT
                     \"SAL\",\"TYE\",\"DRU\",\"COM\",\"CAE\",\"CNP\"
               FROM 
     ...
     $contests_contrat = pg_query($query_contrat) or die('Query failed: ' . pg_last_error());
      $cot_salaire='';
      $cot_contrat='';
      $cot_duree='';
      $cot_avantage='';
      $cot_categorie='';
      $cot_cnps='';
     while($rowcot=pg_fetch_row($contests_contrat)){

        $cot_salaire=str_replace(',','.',number_format($rowcot[0]) );;
        $cot_contrat=$rowcot[1];
        $cot_duree=$rowcot[2];
        $cot_avantage= html_entity_decode(str_replace("-","",utf8_encode($rowcot[3])));
     ...
     $query_dpt="
                ...
                WHERE postab.\"IPO\"='".strtoupper($fpo_fonction)."'
 ...
 : ")),0,0  );
        ...
        $pdf->Cell(40,6,strtoupper( utf8_decode($fpo_fonction )),0,1);

        $pdf->Cell(70,8,strtoupper( utf8_decode("N° CNPS : ")),0,0  );
        $pdf->Cell(40,8,strtoupper( utf8_decode($cot_cnps )),0,1);
       ...
       // $pdf->Cell(90,8,strtoupper( utf8_decode($nom_manager.' '.$prenom_manager )),0,1);
        ...
        $pdf->Cell(40,8,strtoupper( $cot_salaire),0,1); 
        ...
        $pdf->Line(10, 200, 200,200); //haut
        $pdf->Line(10, 200, 10,255); //gauche
        $pdf->Line(200, 255, 200,200); //droite
        $pdf->Line(10, 255, 200,255); //bas
        ...
        $pdf->Cell(40,8,strtoupper( utf8_decode($cot_contrat )),0,1);

         $pdf->Cell(70,8,strtoupper( utf8_decode('DUREE DU CONTRAT  : ')),0,0  );
        $pdf->Cell(40,8,strtoupper( utf8_decode($cot_duree )),0,1);

        $pdf->Cell(70,8,strtoupper( utf8_decode('DEPARTEMENT  : ')),0,0  );
        $pdf->Cell(40,8,strtoupper( utf8_decode($dpt_poste )),0,1);

        $pdf->Cell(70,8,strtoupper( utf8_decode('AVANTAGES  : ')),0,0  );
        $pdf->Cell(40,8,strtoupper( utf8_decode($cot_avantage )),0,1);

         $pdf->Cell(70,8,strtoupper( utf8_decode('CATEGORIE  : ')),0,0  );
        $pdf->Cell(40,8,strtoupper( utf8_decode($cot_categorie )),0,1);
