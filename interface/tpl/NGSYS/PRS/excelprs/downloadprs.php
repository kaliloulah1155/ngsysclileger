...
if(isset($export)){
    ...
    $active_sheet->setCellValue('C2',"LISTE DU PERSONNEL");
   ...
   $active_sheet->setCellValue('B4','NOM');
   $active_sheet->setCellValue('C4','PRENOMS');
   $active_sheet->setCellValue('D4','E-MAIL');
   $active_sheet->setCellValue('E4','TELEPHONE');
   $active_sheet->setCellValue('F4',"DATE D'ENTREE");

   $active_sheet->getStyle("A4:F4")->getFont()->setSize(11);
   $active_sheet->getStyle("A4:F4")->getFont()->setBold(true);

   //coleur de fond et coleur 
   $active_sheet->getStyle("A4:F4")->applyFromArray($tableHead);
   $active_sheet->getStyle("A4:F4")->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
   ...
   foreach($data as $lrow){
     ...
     $active_sheet->setCellValue('F'.$count,$lrow[5]);

     $active_sheet->getStyle('A'.$count.':F'.$count)->applyFromArray($styleArray);

     if($count%2==0){
         $active_sheet->getStyle('A'.$count.':F'.$count)->applyFromArray($evenRow);
     }else{
        $active_sheet->getStyle('A'.$count.':F'.$count)->applyFromArray($oddRow);
     }
   ...
   $active_sheet->setTitle("PERSONNELS"); //Renomme le worksheet(onglet)
