...
header('Expires: Mon, 26 Jul 1997 05:00:00 GMT');
    ...
    require('../../../../../configuration_w/PDF/fpdf.php');
     ...
     $numposeidon= $_GET['numposeidon'] ;
      ...
      $query =" SELECT  \"NOM\",\"PRE\",\"DNS\",\"LNS\",\"NAT\",\"NPI\",\"NTP\",\"SIF\",\"NEF\",\"CNE\",\"TEL\",\"BP\",\"DEI\",\"FON\",\"CNP\",\"MAT\",\"TYE\",\"DUR\",\"DPT\",\"AVT\",\"CAT\",\"DCR\" ,\"NMG\",\"PMG\",\"SLT\",\"RIB\",\"BQE\",\"PAC\",\"NTC\",\"FLN\" FROM public.pos_tab_index_prs WHERE  \"NUD\" ='".$numposeidon."'";
     ...
     while ($row = pg_fetch_row($contests)) {
        ...
        $pdf->Cell(55);
        $pdf->Cell(80,5,strtoupper( utf8_decode('N°: '.$numposeidon.'/NGSER/'.$date)),0,1,'C',0 );
        $pdf->Ln(5);
        $pdf->Cell(70,7,strtoupper( utf8_decode('NOM ET PRÉNOMS  : ')),0,0  );
        $pdf->Cell(35,7,strtoupper( utf8_decode($nom.'  '.$prenoms )),0,1);
         
        $pdf->Cell(70,7,strtoupper( utf8_decode('DATE DE NAISSANCE  : ')),0,0  );
        $pdf->Cell(35,7,strtoupper( utf8_decode($datenaiss )),0,0);
        $pdf->Cell(20,7,strtoupper( utf8_decode('LIEU  : ')),0,0  );
        $pdf->Cell(60,8,strtoupper( utf8_decode($lieu)),0,1);

        $pdf->Cell(70,8,strtoupper( utf8_decode('NATIONALITÉ : ')),0,0  );
        $pdf->Cell(35,8,strtoupper( utf8_decode($nationalite )),0,1);

        $pdf->Cell(70,8,strtoupper( utf8_decode("N° DE PIÈCE D'IDENTITÉ : ")),0,0  );
 ...
 : ")),0,0  );
        ...
        $pdf->Cell(70,8,strtoupper( utf8_decode("N° DE PORTABLE : ")),0,0  );
        $pdf->Cell(40,8,strtoupper( utf8_decode($tel )),0,1);

         $pdf->Cell(70,6,strtoupper( utf8_decode("BOITE POSTALE : ")),0,0  );
        $pdf->Cell(40,6,strtoupper( utf8_decode($bp )),0,1);

        $pdf->Cell(70,6,strtoupper( utf8_decode("DATE D'ENTRÉE : ")),0,0  );
        $pdf->Cell(40,6,strtoupper( utf8_decode($dateentree )),0,1);

        $pdf->Cell(70,6,strtoupper( utf8_decode("POSTE : ")),0,0  );
        ...
        $pdf->Cell(40,8,strtoupper( utf8_decode($nom_manager.' '.$prenom_manager )),0,1);
        ...
        $pdf->Cell(70,8,strtoupper( utf8_decode("PERSONNE Á CONTACTER : ")),0,0  );
        $pdf->Cell(40,8,strtoupper( utf8_decode($nom_contact )),0,1);

        $pdf->Cell(70,8,strtoupper( utf8_decode("TÉLÉPHONE DU CONTACT : ")),0,0  );
        ...
        $pdf->Line(10, 200, 197,200); //haut
        $pdf->Line(10, 200, 10,250); //gauche
        $pdf->Line(197, 250, 197,200); //droite
        $pdf->Line(10, 250, 197,250); //bas
        ...
        $pdf->Cell(40,8,strtoupper( utf8_decode($type_contrat )),0,1);
         ...
         $pdf->Cell(70,8,strtoupper( utf8_decode('DURÉE DU CONTRAT  : ')),0,0  );
        ...
        $pdf->Cell(40,8,strtoupper( utf8_decode($avantage )),0,1);
         ...
         $pdf->Cell(70,8,strtoupper( utf8_decode('CATÉGORIE  : ')),0,0  );
