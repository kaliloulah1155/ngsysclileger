<?php
   ...
   // echo 'Date contrat : '. $categorie; exit;
    ...
    $dtemb = explode("/", $date_embauche);
      $dtembauche=      (new DateTime($dtcontrat))->format('Y-m-d') ;
         ...
         $prime_anc=intval($salaire_base)*(intval($years)+1)/100;
 ...
 //  echo $dtcontrat.' '.$categorie.' '.$salaire_base.' '.$dtembauche.' '.$prime_anciennete.' '.$prme_rdt;
    ...
    /////////////// INSERT DATA INSIDE REGISTER OF EMPLOYEE //////////////////////
       ...
       //date('d M Y-M-d', strtotime($string))

 
		  pg_query("UPDATE public.pos_tab_index_reg
                SET 
                    \"DCL\"='".strval($dtcontrat)."',
					\"CAE\"='$categorie',
                    \"SAB\"='".intval($salaire_base)."',
