 ...
 */
  /*
   *  Attention Copie du code concernant les listes hierarchiques 
   */
/*
...
function arbreDossier(idArbreHier, questionBib, codeRub, bDerniersniveau, titre, largeur, hauteur, bPositionBas, bFenAttente) 
{	
	...
	{		
		arbreHierarchiqueReq.open("GET", '../../../../interface/session/principal/ajax/recup_arbo_dossier.php?POS_QUESTION=' + questionBib, false);	
		arbreHierarchiqueReq.onreadystatechange = function() {handleArbreDossier(idArbreHier, codeRub, bDerniersniveau, titre, largeur, hauteur, bPositionBas, bFenAttente);}; 
...
function handleArbreDossier(idArbreHier, codeRub,  bDerniersniveau, titre, largeur, hauteur, bPositionBas, bFenAttente) {	
	...
	{ 
		...
		if(objetRetour.code_retour == 1)
		{
			...
			suggest += ' class="lienHierarchique '+niveauStyle+' "  >'+ titre + '<img src="../../../../images/icons/suppri_blanc.png" style="cursor: pointer;"  onclick="javascript:closeArbreHierarchique(\'' + idArbreHierencours + '\');" title="Fermer" align="right"/></div>';							
			...
			{													
				...
				function lien(dernierNiveauStyle,decalage,niveauStyle,idArbreHier,codeRub,i)
				{	
					...
					lien += ' style="cursor: pointer;'+decalage+';" onclick="javascript:setCodeDossier(\'' + idArbreHier + '\',\'' + codeRub + '\',\''+ i+'\');" ';						
					return lien;
				}
				...
				//		niveauStyle='dernierNiveauHierarchique';							
						suggest += lien(dernierNiveauStyle,decalage,niveauStyle,idArbreHier,codeRub,i);
					}
			...
			//			niveauStyle="dernierNiveauHierarchique";													
					...
					suggest += lien('',decalage,niveauStyle,idArbreHier,codeRub,i);
			...
			{			
				...
				if (!bPositionBas || bPositionBas=='' || bPositionBas==1)
					...
					var haut=-position_haut;   //position de y en relatif 25:elle s'ouvre en dessous -270:elle s'ouvre au dessus
				
				var gauche=-2; //position de x en relatif
				
				//alert (document.getElementsByName(listenameinput[0])[0].name);
				var element=document.getElementsByName('POS_VAL_RUB_'+codeRub)[0];
                
                //var xy = YAHOO.util.Dom.getXY('id_rub_'+codeRub);
                //xy[1] += document.getElementById('id_rub_'+codeRub).offsetHeight;
                //YAHOO.util.Dom.setXY(idArbreHier, xy);
...
//Click function
function setCodeDossier(idArbreHier, codeRub, j) 
{ 
	...
	var objetRetour = eval('(' + arbreHierarchiqueReq.responseText + ')');
    var valeur = objetRetour.voca[j][2];	
    document.getElementsByName('POS_VAL_RUB_'+codeRub)[0].value = valeur.substring(0, valeur.indexOf(" | "));	
} 