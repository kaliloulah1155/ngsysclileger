...
//			portletDrag.destroy();
	...
	PosYUIPortal.prototype.makeLayoutGroup = function(group, layoutDefinition) {
		...
		if(elements_dans_bal == "0")
			zonesList = this.removePortletByType("bal", zonesList);
		var groupNode = Y.Node.create('<ul class="' + groupClass + '"/>');
		groupNode.setData('group', group);
		if (layoutDefinition.type == 'vertical') {
			groupNode.setStyle('width', group.weight * 95 / layoutDefinition.weightsum + '%');
		} else if (layoutDefinition.type == 'horizontal') {
			groupNode.setStyle('height', group.weight * 95 / layoutDefinition.weightsum + '%');
		}
		new Y.DD.Drop({
			node : groupNode,
			bubbles : Y.PosYUIPortal
