...
function MenuItemClicked(entryNo, fromToggle) {
	...
	var b = theBrowser;
	// -----------------------------------
	// NB : dans le cas de l'affichage du contenu d'un dossier dans la frame centrale
	// on ne passe pas par cette fonction, car c'est le href contenu dans le titre qui prend la main
    // si on passe pas cette fonction c'est que l'utilisateur a clique sur l'icone du dossier
    // donc il ne faut pas fermer la fiche d'index en cours
    // ----------------------------------- 
	//si on a une fiche d'index ouverte on la ferme
	if (theMenu.entry[entryNo].type.indexOf("Folder") == -1 && theMenu.entry[entryNo].type.indexOf("FinDossier") == -1)
	{
		if(!parent.testOpenDocument(false))
	       return false;
    }
