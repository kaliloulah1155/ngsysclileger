...
// D�finition des diff�rents images pour les BAL
...
function changerProfil(szProfil) {
    if (parent.testOpenDocument(true)) {
        parent.closeCenterOnglet();
...
/*
 *	Cette fonction ressemble � une fonction r�cursive mais n'est pas utilis�e
 *	dans ce but, on attend juste que objHttp.readyState soit �gal � 4 
 *	(c�d on attend le nb de r�ponses de la BAL) pour lancer la recherche sur la BAL suivante
 *  Comme la reponse est asynchrone, si on utilise un code s�quentiel, les derni�res r�ponses
 * peuvent arriver en premier et ce n'est pas g�rable avec un seul objet de type XMLHttpRequest
 */
function loadBal(i) {
    ...
    if (i < tabBal.length) {
        ...
        for (j = 0; j + iBase < tabBal.length; j++) {
            ...
            // on teste si la valeur est (...) pour lancer la recherche ou si la question a d�j� �t� pos�e
...
function reloadBal() {
    // peut-�tre long selon les navigateurs et le nombre de bal
