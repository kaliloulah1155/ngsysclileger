...
function connexion()
{
	...
	{
        document.principal.POS_PASSWD.value = crypt_text(document.principal.POS_PASSWD.value, szCleCrypt);
    }
...
function init() {
	...
	dlgChangeMdp.render();
	// contournement pour éviter que le formulaire s'affiche et disparaisse au chargement
	document.getElementById("containter-dialog1").style.display = "";
}
...
var handleSubmit = function() {
    ...
    {
        alert("Vous n'avez pas saisi deux fois le même mot de passe");
        return;
    }
    else
    {
...
{	
    var bRetour = false;
	// les profils ne sont pas affichés : on les demande au serveur
	if (document.getElementById("div_profil").style.display == "none")
	{
	    if (searchReq.readyState == 4 || searchReq.readyState == 0)
		{
			...
			if (searchReq.readyState == 4 || searchReq.readyState == 0)
			{
			    ...
			    if (objAction.code_retour == 1)
        	   	{
					// suppression de la première option
        	   	...
        	   	{
        	   	    alert("Votre mot de passe a expiré. Vous devez le changer.");
        	   	    dlgChangeMdp.show();
        	   	    document.principal.POS_UTIL.readOnly = true;
                    document.principal.POS_UTIL.className = "verrouille";
					document.principal.POS_PASSWD.readOnly = true;
	...
	else
	{
		if(document.principal.POS_PROFIL.selectedIndex < 0)
			alert("Veuillez sélectionner votre profil de connexion");
		else
		  document.principal.submit();
	}
	return bRetour;
