...
{
	var iNbOnglets = document.querySelectorAll("li[id^='label_onglet_']").length;
    for(i=0; i<iNbOnglets; i++)
	{
		var objOnglet = document.getElementById("ONGLET_"+i);
		var objLabelOnglet = document.getElementById("label_onglet_"+i);
	 
		if(i == indOnglet)
		{
        	objOnglet.style.display = "";
        	if (objLabelOnglet)
        	   objLabelOnglet.className = "titreongletsel";
        }
		else
		{
        	objOnglet.style.display = "none";
        	if (objLabelOnglet)
        	   objLabelOnglet.className = "titreongletnotsel";
        }
