...
<xsl:choose> 
  <xsl:when test="contains( $text, $CR)"> 
    ...
    <xsl:element name="br"/> 
    <xsl:call-template name="str:replace_retour_chariot"><!-- recurse --> 
...
<xsl:variable name="smallcase" select="'aàbcdeéèêfghijklmnopqrstuùûvwxyz'" />
<xsl:variable name="uppercase" select="'AABCDEEEEFGHIJKLMNOPQRSTUUUVWXYZ'" />
   
<func:function name="str:toupper">
	<xsl:param name="string" select="''" />
    <func:result select="translate($string, $smallcase, $uppercase)" />
</func:function>
</xsl:stylesheet>
